#!/bin/bash
# setup_kiro_cli_debian.sh
# Installs Amazon Kiro CLI (AI-powered development assistant)
...
handle_error() {
    ...
    am start -a android.intent.action.VIEW \
        -d "nativecode://callback?result=failure&name=ai_tools_kiro_cli" \
        > /dev/null 2>&1 || true
...
echo "NativeCode: Setting up Kiro CLI..."
echo "=================================================="
...
# Kiro CLI requires Node.js. Same pattern as Codex.
# ────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────
# Step 2: Install Kiro CLI globally via npm
# Amazon Kiro CLI is distributed as an npm package.
# ────────────────────────────────────────────────────────────────
echo ""
echo "NativeCode: Installing @anthropic-ai/kiro..."

export npm_config_unsafe_perm=true

sudo npm install -g @anthropic-ai/kiro || handle_error "Kiro CLI npm install"

echo "NativeCode: Kiro CLI installed."

# ────────────────────────────────────────────────────────────────
# Step 3: Verify the install
# ────────────────────────────────────────────────────────────────
echo ""
echo "NativeCode: Verifying Kiro CLI..."

KIRO_BIN=""
if command -v kiro > /dev/null 2>&1; then
    KIRO_BIN="$(which kiro)"
elif [ -f "$NODE_ROOT/bin/kiro" ]; then
    KIRO_BIN="$NODE_ROOT/bin/kiro"
    ln -sf "$KIRO_BIN" /usr/local/bin/kiro 2>/dev/null || true
fi

if [ -n "$KIRO_BIN" ]; then
    echo " [✅] Kiro CLI found at $KIRO_BIN"
else
    echo " [⚠️] kiro binary not found in PATH — check npm global prefix"
fi
...
touch "/home/$TARGET_USER/.nativecode/kiro_cli_installed"
...
echo ""
echo "=================================================="
echo "🎉 NativeCode: Kiro CLI Setup Complete!"
echo "=================================================="
echo " Tool    : Kiro CLI (Amazon AI Dev Assistant)"
echo " Command : kiro"
echo " Usage   : kiro  (interactive)  |  kiro init"
echo ""
echo "Note: Configure your credentials with:"
echo "  kiro auth login"
echo "=================================================="
echo ""
...
am start -a android.intent.action.VIEW \
    -d "nativecode://callback?result=success&name=ai_tools_kiro_cli" \
    > /dev/null 2>&1 || true